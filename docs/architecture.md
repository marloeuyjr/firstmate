...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while herdr combines native agent state with its ANSI-aware structural composer classifier under the adapter-specific rules in [Current transport behavior](herdr-backend.md#current-transport-behavior).
...
The same OpenCode busy-queue case remains a known gap on the Herdr adapter and is recorded in [Active limits](herdr-backend.md#active-limits).
...
Teardown is fail-closed for ship worktrees and refuses rather than discards work that its safety checks do not accept.
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the pre-check normalization, landed-work proofs, PR-discovery fallback, and stale-lock recovery procedure.
